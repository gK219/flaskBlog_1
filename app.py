...
class Posts(db.Model):
	...
	subtag = db.Column(db.String(80), nullable=False)
...
@app.route('/index')
def home():
	flash("thanks for visiting here")
		...
		if request.method=='POST':
			...
			if (sno == '0'):
				post = Posts(title=box_title, slug=slug, content=content,
							 subtag=tline, img_file=img_file, date=date)
				db.session.add(post)
				db.session.commit()
				flash("Your new post is Uploaded successfully",'success')
			else:
				...
				db.session.commit()
				flash("Your  post is updated & Uploaded successfully",'success')
		post=Posts.query.filter_by(sno=sno).first()
...
def contact():
	if (request.method =='POST'):
		...
		#					body=message + "\n" + phone)
		flash("thanks for visiting here",'success')
